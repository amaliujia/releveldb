...
class MmapLimiter {
public:
  MmapLimiter(const MmapLimiter&) = delete;
  MmapLimiter& operator=(const MmapLimiter&) = delete;
...
public:
  bool Insert(const std::string& name) {
    MutexLock l(&mu_);
    return locked_files_.insert(name).second;
  }

  void Remove(const std::string& name) {
    MutexLock l(&mu_);
    locked_files_.erase(name);
  }
...
public:
  ...
  virtual Status NewSequentialFile(const std::string& name,
                                    SequentialFile** ret) {
    ...
    if (f == nullptr) {
      ...
      *ret = new PosixSequentialFile(name, f);
      return Status::OK();
    }
  }

  virtual Status NewRandomAccessFile(const std::string& name,
    ...
    *ret = nullptr;
    ...
    if (fd < 0) {
      ...
      uint64_t size;
      s = GetFileSize(name, &size);
      if (s.ok()) {
        void* base = mmap(NULL, size, PROT_READ, MAP_SHARED, fd, 0);
        if (base != MAP_FAILED) {
          *ret = new PosixMmapReadableFile(name, base, size, &mmap_limit_);
        } else {
          s = IOError(name, errno);
        }
      }
      ...
      *ret = new PosixRandomAccessFile(name, fd);
    }
    return s;
  }
